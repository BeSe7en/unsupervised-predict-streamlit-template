...
def preprocess_tweet(tweets):
	...
	def handle_retweet(tweet):
		...
		return handle
	tweets['retweet_handle'] = tweets['message'].apply(handle_retweet,1)
...
def preprocess_csv(tweets):
	...
	#function to determine if tweet is original or retweet
	def is_retweet(tweet):
		word_list = tweet.split()
		if "RT" in word_list:
			return 1
	...
	#function to extract mentions within the tweet
	def get_mentions(tweet):
		mentions = []
		word_list = tweet.split()
...
def main():
	...
	if selection == "Model Explainations":
		options = ['Logistic Regression','Linear Support Vector Classifier','Random Forest Classifier','Gaussian Naive Bayes Classifier','Multinomial Naive Bayes Classifier','XGBoost Classifier']
	...
	if selection == "Explore the data":
		options =  ['Dataset','Distribution of data per sentiment class','Proportion of retweets, hashtags and original mentions','Popular retweet handles per sentiment group in a word cloud', 'Popular hashtags in per sentiments group','Popular mentions per sentiment group']
		...
		if selection == 'Proportion of retweets, hashtags and original mentions':
			st.subheader('Proportion of retweets, hashtags and original mentions')
			...
			st.markdown('Furthermore, we see that  82% of our data contains no hashtags. While the hashtags that is being used could still provide a great starting point for the model, most of the data does not contain hashtags and as such, this feature might prove less useful than one might hope.')
			st.markdown('Lastly, let\'s consider the mentions. For our purposes, we only look at original mentions, meaning that we don\'t consider handles that gets retweeted to count towards the total. We see that even though the majority of data does not contain original mentions, there still exist a substantial amount within our data, and will therefore be important to consider within our model.')
	...
	if selection == 'Prediction':
		...
		if pred_type == "Multiple Tweets":
			...
			if tweets_csv is not None:
				...
				df_raw = pd.DataFrame(df_uploaded['message'], columns=['message'])
				processed_df = preprocess_csv(df_raw)
				X_pred = processed_df['message']
			
			options = ["Logistic Regression Classifier", "Linear Support Vector Classifier"," Gaussian Naive Bayes Classifier"]
			selection = st.selectbox("Choose Your Model", options)
			...
			if st.button("Classify CSV"):
				...
				if selection == "Logistic Regression Classifier":
					...
					lsvc = pickle.load(open('resources/LinearSVC.pkl','rb'))
					predictor = lsvc
				elif selection == "Gaussian Naive Bayes Classifier":
					gnb = pickle.load(open('resources/GaussianNaiveBeyes.pkl','rb'))
					predictor = gnb	
